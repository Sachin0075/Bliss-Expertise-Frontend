        ...
        <div className="orders-page">
            ...
            {orders.length === 0 ? (
                ...
                <div className="orders-list space-y-6">
                    {orders.map((order) => (
                        <div key={order._id} className="order-card border p-4 rounded-lg shadow-md">
                            <h3 className="text-2xl font-semibold mb-4">Order ID: {order._id}</h3>
                            ...
                            <div className="cart-items space-y-4">
                                {order.cartItems.map((item) => (
                                    <div key={item._id} className="cart-item flex justify-between items-center border p-2 rounded-md">
                            ...
                            <p className="text-lg font-semibold mt-4">Total Amount: ₹{order.totalAmount}</p>
                            <p className="text-sm text-gray-500 mt-2">Order Date: {new Date(order.createdAt).toLocaleDateString()}</p>
                        </div>
                    ))}
                </div>
            )}
