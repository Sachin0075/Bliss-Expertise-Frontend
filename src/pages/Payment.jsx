...
const Payment = () => {
    ...
    const handlePayment = () => {
        ...
        if (paymentMethod === 'cod') {
            ...
            setTimeout(() => { generateAndDownloadBill() }, 15000)
            navigate('/RazorpayPayment', { state: { finalTotalAmount, userData, cartItems } });
        }
    };
    ...
    const generateAndDownloadBill = () => {
        ...
        const productTotalAmount = cartItems.reduce((total, item) => total + (item.price * item.quantity), 0);
        ...
        doc.text('Ajay Kumar, Opposite to Janatha Lunch Home, B.V Road, Kankandy, Mangaluru-575001', 14, yOffset + 10);
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex items-center justify-center">
            <div className="bg-white shadow-lg rounded-lg p-8 w-full max-w-md">
                <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800">Select Payment Method</h2>
                ...
                <div className="mt-4 text-center">
                    <Link to="/cart" className="text-blue-500 underline">Back to Cart</Link>
                </div>
            </div>
        </div>
    );
